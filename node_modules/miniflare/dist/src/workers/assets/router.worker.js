...
var src_default = {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      return env.CONFIG.invoke_user_worker_ahead_of_assets ? env.JAEGER.enterSpan("invoke_user_worker_first", async () => {
        if (!env.CONFIG.has_user_worker)
          throw new Error(
            "Fetch for user worker without having a user worker binding"
          );
        return env.USER_WORKER.fetch(maybeSecondRequest);
      }) : env.CONFIG.has_user_worker ? env.JAEGER.enterSpan("has_user_worker", async (span) => await env.ASSET_WORKER.unstable_canFetch(request) ? (span.setTags({
        asset: !0,
        dispatchType: "asset" /* ASSETS */
      }), analytics.setData({ dispatchtype: "asset" /* ASSETS */ }), env.ASSET_WORKER.fetch(maybeSecondRequest)) : (span.setTags({
        asset: !1,
        dispatchType: "worker" /* WORKER */
      }), analytics.setData({ dispatchtype: "worker" /* WORKER */ }), env.USER_WORKER.fetch(maybeSecondRequest))) : env.JAEGER.enterSpan("assets_only", async (span) => (span.setTags({
        asset: !0,
        dispatchType: "asset" /* ASSETS */
      }), analytics.setData({ dispatchtype: "asset" /* ASSETS */ }), env.ASSET_WORKER.fetch(request)));
    } catch (err) {
