...
let currentEpicData: any = null;
let currentFeaturesData: any[] = [];
let currentStoriesData: Record<number, any[]> = {};

const API_BASE = "/api";  // Cloudflare worker route

function getApiChoice(): string {
  return (document.querySelector('input[name="apiChoice"]:checked') as HTMLInputElement)?.value || "google_gemini";
}

async function createEpicOnly() {
  const reqTitle = (document.getElementById("reqTitle") as HTMLTextAreaElement).value.trim();
  const reqDescription = (document.getElementById("reqDescription") as HTMLTextAreaElement).value.trim();
  const reqInstructions = (document.getElementById("reqInstructions") as HTMLTextAreaElement).value.trim();
...
// Utility
function showSpinner(visible: boolean, msg: string = "Loading...") {
