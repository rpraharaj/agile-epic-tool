...
interface AddSingleStoryRequestBody {
  featureData?: string;       // JSON string representing the Feature
  existingStories?: string;   // JSON array string of existing stories
  apiChoice?: string;
}

export async function addSingleStoryHandler(request: Request, env: any): Promise<Response> {
  const body = (await request.json()) as AddSingleStoryRequestBody;
  ...
  const prompt =
    `We have a Feature and existing Stories. Generate exactly ONE new Story...`;
